...
"""Example / benchmark for building a Lyric LSTM model.
...
# from tensorflow.models.rnn.Lyric import reader
import reader
...
class LyricModel(object):
  ...
  def __init__(self, is_training, config, input_, embedding):
    ...
    # when the state is modified, self._initial_state is modified!
...
#         "softmax_w", [size, vocab_size], dtype=data_type())
    ...
    self._cost = cost = tf.reduce_sum(loss) / batch_size # tensor ops to calculate the cost
    
    self._final_state = state # tensor opts to calculate the final state.
    ...
    # compute gradient and apply gradient clip.
    grads, _ = tf.clip_by_global_norm(tf.gradients(cost, tvars),
                                      config.max_grad_norm)
    optimizer = tf.train.GradientDescentOptimizer(self._lr)
    
    # apply gradient descent to training variables.
    # This step also define the training ops, which essentially update training variables.
    # the training variables here are the weights and biases of the neural network.
    ...
    self._lr_update = tf.assign(self._lr, self._new_lr) # assign new learning rate to old one.

  def assign_lr(self, session, lr_value):
  # function to feed new learning rate into the session.
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      for i in range(config.max_max_epoch):
        ...
        # update the training model's learning parameter.
